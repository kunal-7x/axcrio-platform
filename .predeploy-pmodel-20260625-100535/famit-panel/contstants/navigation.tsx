...
export const navigation = [
    ...
    },
    {
        ...
        list: [
            ...
            // Agent Tools — tenant surface of the Tolex agent-tooling system: grant + gate what the
            // voice agent can DO (scoped to this tenant's own campaigns).
            { title: "Agent Tools", href: "/agent-tools", feature_key: "grow.agent_tools" },
        ],
    },
    {
    ...
    {
        // PROPERTY STUDIO — turn a 2D floor plan (or a text brief / built-in sample)
        // into an interactive 3D property model with a public, customer-facing share
        // link the voice agent can text mid-call. A single page (its three input modes
        // + the viewer live in-page), so a top-level LINK. UNKEYED → always visible;
        // the page self-gates via the backend FEATURE_PMODEL flag (it renders a calm
        // DormantCard when the feature is off), so no entitlement key is needed. `cube`
        // is a real Icon-registry key and reads as the 3D model engine.
    ...
    {
        // CREDITS — the credit-wallet hub (renamed from "Money"). ONE rail entry → /credits,
        // a tabbed hub (Wallet / Buy Credits / Usage / Pricing) that FOLDS IN the billing tabs
        // (Overview / Vendors / Spending / Plan / Audit / Payments) via the shared hub strip.
        // The old /billing/* + /payments routes still resolve (no 404s). UNKEYED so the CORE
        // wallet surface can never be hidden by an entitlement (per-tab hide/lock + backend
        // 404/402 are the real boundary).
        title: "Credits",
        icon: "wallet",
        href: "/credits",
    },
    ...
    {
        // FAMIT RESEARCH — the premium "instrumented conversation science" lab: a single top-level
        // link (the page owns its own tabs — Overview / Call Detail / Outcomes Lab). It surfaces
        // call-level affect/prosody DYNAMICS, so it is `roles:"manager"` (not a read-only-agent
        // surface); `feature_key:"mod.research"` lets a super-admin gate it as a high-end add-on
        // (resolveNav drops/locks it on HIDE/LOCK). A top-level LINK that resolves to LOCK is NOT
        // dropped (only HIDE drops it), so the link always opens the real page. Icon "chart" is a
        // real key in components/Icon (the science/analytics glyph).
        title: "Famit Research",
        icon: "chart",
        href: "/research",
        roles: "manager",
        feature_key: "mod.research",
    },
    {
