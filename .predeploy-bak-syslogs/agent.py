...
def _build_call_llm(primary_key: str):
    ...
    try:
        ...
        if use_fb and _LkLLMFallback is not None and len(keys) > 1:
            # primary first (load spread), then the rest (failover order)
            ordered = [primary_key] + [k for k in keys if k != primary_key]
            members = [_one(k) for k in ordered]
            # OPTIONAL last resort: a separate fast model (its OWN quota) for the rare case
            # where EVERY key is rate-limited on the primary model. Off unless configured.
            ...
            logger.info("FF1 LLM failover ON: %d keys%s", len(ordered),
                        (" + fallback model " + fb_model) if fb_model else "")
            return adapter
    except Exception as exc:  # noqa: BLE001 — never let failover wiring break the earner
        logger.warning("FF1 LLM failover wiring failed -> single key: %r", exc)
