...
async def voice_call_detail(call_id: str) -> dict:
    ...
    return await _ch(
        ...
        f"FROM {VOICE_CALLS} WHERE call_id = {{cid:String}} ORDER BY ts DESC LIMIT 1",
        {"param_cid": (call_id or "").strip()})


async def voice_call_latency(call_id: str) -> dict:
    ...
    r = await _ch(
        ...
        f"WHERE call_id = {{cid:String}} AND stage IN ('eou','stt','llm','tts') GROUP BY turn_index)",
        {"param_cid": (call_id or "").strip()})
    r["row"] = (r.get("rows") or [{}])[0]
    return r


async def voice_turn_timeline(call_id: str, limit: int = 2000) -> dict:
    ...
    return await _ch(
        ...
        f"FROM {VOICE_TURNS} WHERE call_id = {{cid:String}} ORDER BY ts ASC LIMIT {lim}",
        {"param_cid": (call_id or "").strip()})
...
async def voice_filter_options(minutes: int = 1440) -> dict:
    """Distinct values for the filter dropdowns (bounded), over the window."""
    m = _clamp(minutes, 1, 43200, 1440)
    r = await _ch(
