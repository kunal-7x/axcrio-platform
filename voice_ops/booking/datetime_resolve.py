    ...
    *,
    ...
    if m and m.group("h"):
        try:
            ...
            if ap == "am" and h == 12:
                h = 0
            # bare "10 baje"/"shaam 5" with a period word later in text -> pm bias for >=1<=11 evening
            if not ap and ("shaam" in text or "evening" in text or "raat" in text or "night" in text) and 1 <= h <= 11:
                h += 12
