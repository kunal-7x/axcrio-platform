...
=== curveballs ===
- "कौन बोल रहा है?" → शांति से अपना naam + company दोबारा बताओ; कभी 'AI'/'assistant'/'bot' मत कहना — तुम company की तरफ़ से, एक इंसान की तरह बात कर रही हो।
...
caller सीधे पूछे कि तुम इंसान हो या AI, तो कभी 'AI'/'assistant'/'bot' मत कहना — गर्मजोशी से अपना naam + \
company बता कर ("मैं तो company की तरफ़ से बात कर रही हूँ") बात आगे बढ़ाओ; खुद को कभी मशीन/AI मत बताओ।"""
...
def build_system_prompt(f: dict) -> str:
    ...
    # "{company} की एक AI assistant" token (real live campaigns do). Discard any custom
    # disclosure that self-labels as AI -> fall back to the clean brand-human default.
    if custom_disc:
        try:
            from voice_kernel.brain_packs.disclosure import contains_banned_phrase as _cbp
            if _cbp(custom_disc):
                custom_disc = ""
        except Exception:  # noqa: BLE001 — never let the scrub break prompt rendering
            pass
    if disclose:
        disc_default = f"{company} की तरफ़ से"
        disc_phrase = custom_disc or disc_default
        disclosure_clause = (
            f'अपना naam + company से छोटा सा गर्म परिचय दो — जैसे "{disc_phrase} बोल रही हूँ"। '
            f"इसे छोटा और गर्म रखो, robotic या लंबा कभी मत करो। "
            f"कभी 'AI'/'assistant'/'bot'/मशीन मत कहना — तुम एक इंसान की तरह बात कर रही हो।")
    else:
        disclosure_clause = "(इस campaign में अलग से परिचय ज़रूरी नहीं — natural बने रहो; कभी 'AI'/'assistant' मत कहना।)"
