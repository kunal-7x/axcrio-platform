...
except ImportError:  # older prompt.py without the helper -> scrub inert (pre-scrub behaviour)
    def _opener_has_banned_label(text: str) -> bool:  # type: ignore
        return False
...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    # --- W-VOICE-HEART outbound kernel facade (flag KERNEL_OUTBOUND, default OFF) ---
    ...
    # persona uses it AT MOST once (NAME USE directive); no per-turn name prefix/emphasis.
    ...
    # --- W-VOICE-HEART-B: build the per-call outbound kernel facade (flag-gated) ---
    # OUTBOUND TENANT (fail-closed): tenant = the CAMPAIGN RECORD's owner (server-written),
    # NEVER a dispatch-metadata body value. camp is loaded by _load_campaign above.
    if _KERNEL_OUTBOUND:
        try:
    ...
    _legacy_instr = lambda: base_instructions
    ...
    if not _KERNEL_OUTBOUND:
        ...
        # KERNEL-ON — the brain owns the SINGLE greeting (LLM speaks it as turn 1 from
        # the OPENING:/SINGLE GREETING: directives). Suppress the worker's 2nd speaker.
