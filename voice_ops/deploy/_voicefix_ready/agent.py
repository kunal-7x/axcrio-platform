...
def _llm_opener(agent_name: str, company: str, product: str, lead_name: str,
    ...
    # AIRTIGHT: a campaign's stored ai_disclosure may itself bake the founder-banned
    # "{company} की एक AI assistant" token. Discard any self-labeling disclosure_phrase
    # before it reaches the fallback line OR the Groq sysmsg -> clean brand-human default.
    _dp = (disclosure_phrase or "").strip()
    if _dp:
        try:
            from voice_kernel.brain_packs.disclosure import contains_banned_phrase as _cbp
            if _cbp(_dp):
                _dp = ""
        except Exception:  # noqa: BLE001 — never let the scrub crash the earner
            pass
    disc_phrase = (_dp or f"{company} की तरफ़ से").strip()
    ...
    try:
        ...
        disc_clause = (f"अपना naam {agent_name} बताओ और छोटा सा गर्म परिचय दो कि तुम "
                       f"{disc_phrase} बोल रही हो (छोटा रखो, robotic नहीं; कभी 'AI'/'assistant'/'bot'/मशीन मत कहना — एक इंसान की तरह), और "
                       if disclose else
                       f"अपना naam {agent_name} बताओ और natural रहो (कभी 'AI'/'assistant' मत कहना), फिर ")
        sysmsg = (
        ...
        if not text:
            return fallback
        # AIRTIGHT OUTPUT-BOUNDARY SCRUB (founder #1 rule): even a successful Groq
        # opener may hallucinate an "AI assistant"/bot/machine self-label. Reuse the
        # kernel's block-list and DISCARD any opener that contains a banned phrase —
        # fall back to the clean brand-human line (disc_phrase = "{company} की तरफ़ से").
        try:
            from voice_kernel.brain_packs.disclosure import contains_banned_phrase
            if contains_banned_phrase(text):
                logger.warning("opener contained a banned self-label, using clean fallback: %r", text)
                return fallback
        except Exception as scrub_exc:  # noqa: BLE001 — never let the scrub crash the earner
            logger.warning("opener scrub unavailable (%r); returning Groq opener", scrub_exc)
        return text
    except Exception as exc:  # noqa: BLE001
