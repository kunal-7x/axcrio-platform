...
# ── W-INT Patch G — box memory wiring for the inbound kernel (flag KERNEL_INBOUND) ──
# ON-BOX only + flag-gated: bind the box's tenant-RLS asession to the kernel so live
# lead-memory persists under RLS. CI/local (flag OFF or no droplet db) => empty mem,
# never an error. This is the ONLY place a droplet db import meets the kernel.
if os.getenv("KERNEL_INBOUND", "0") in ("1", "true", "True"):
    try:
        from db.engine import asession as _box_asession  # box path (caller.py-style: `from db ...`)
        from voice_kernel.integrations import inbound as _ki_boot
        _ki_boot.bind_box_memory(_box_asession)
    except Exception as _exc:  # noqa: BLE001 — bind is best-effort; OFF/CI degrades to empty mem
...
class ManagerAgent(Agent):
    ...
    def __init__(self, *, caller_id: str, tenant_id: str, role: str, is_manager: bool,
                 session_id: str, kernel_facade=None, kernel_on: bool = False) -> None:
        # W-INT Patch C3 — manager persona via kernel (flag KERNEL_INBOUND). OFF => byte-identical.
        _legacy = lambda: _build_instructions(caller_id, is_manager, role)
        if kernel_on and kernel_facade is not None:
            from voice_kernel.integrations import inbound as _ki
            super().__init__(instructions=_ki.assemble_inbound_instructions(
                kernel_facade, legacy_render=_legacy, fields={}, recap="",
                grounding="", pg_memory=""))
        else:
...
def _build_sales_instructions(fields: dict, recap: str, caller_name: str,
    ...
    inbound_override = (
        ...
        "natural casual spoken Hinglish (Hindi words in Devanagari + everyday English loan-words kept "
        "in English — see the SCRIPT RULE at the very end); if they mix (Hinglish), mix the same way. "
        "The MOMENT they switch language at ANY point — switch WITH them on your very next line, "
        "immediately, no lag. There is NO default language and NO house style: you simply follow the "
        "caller, turn by turn. Keep it light and casual either way (a real Indian salesperson on the "
        "phone, never heavy/formal/literary, never long bookish sentences). NEVER announce, explain, ask about, or apologise "
    ...
    lang_lock = (
        ...
        "starting with your first reply. If their last message was in English, reply in clean natural "
        "English. If in Hindi or Hinglish, reply in casual spoken Hinglish (see SCRIPT RULE below). If "
        "they mix, mirror the mix. Switch the MOMENT they switch, on your very next line. The Hindi "
        "text in the knowledge pack above is reference material ONLY and must NOT make you reply in "
        "Hindi when the caller spoke English. Never mention or ask about language — just mirror it.\n"
        "\n=== SCRIPT RULE — HOW TO WRITE HINDI/HINGLISH (non-negotiable, applies ONLY when you are "
        "speaking Hindi or Hinglish; an English caller still gets pure English) ===\n"
        "You are read aloud by a Hindi text-to-speech voice that speaks NATIVE DEVANAGARI cleanly but "
        "MANGLES romanized/typed-in-Latin Hindi. So when you speak Hindi/Hinglish:\n"
        "1. Write every HINDI word in DEVANAGARI script — never in Roman/Latin letters. BANNED (these "
        "garble the voice): \"hai\", \"hain\", \"mein\", \"ke\", \"ki\", \"ka\", \"ko\", \"aapko\", "
        "\"sakta\", \"sakti\", \"karna\", \"kariye\", \"kya\", \"haan\", \"nahi\", \"acha\", \"thoda\", "
        "\"ruko\", \"milega\", \"chahiye\". WRITE INSTEAD: \"है\", \"हैं\", \"में\", \"के\", \"की\", "
        "\"का\", \"को\", \"आपको\", \"सकता\", \"सकती\", \"करना\", \"करिए\", \"क्या\", \"हाँ\", \"नहीं\", "
        "\"अच्छा\", \"थोड़ा\", \"रुको\", \"मिलेगा\", \"चाहिए\".\n"
        "2. Keep ordinary ENGLISH loan-words in LATIN exactly as a real Indian salesperson says them — "
        "do NOT translate these to Hindi: project, property, unit, BHK, price, budget, discount, "
        "offer, booking, book, confirm, payment, ready, available, details, visit, site visit, "
        "WhatsApp, ok, sir, madam, please, team, time, morning, afternoon, number — plus all brand "
        "names (e.g. Codename Joy 3.0). This natural Devanagari-Hindi + Latin-English mix is exactly "
        "how real Hinglish is spoken AND the only form the voice pronounces cleanly.\n"
        "3. NEVER mix Devanagari and romanized Hindi in the SAME sentence (e.g. do NOT write "
        "\"₹2 करोड़ ke budget mein\" — write \"₹2 करोड़ के budget में\").\n"
        "4. NUMBERS: write as digits (₹2,00,000), not spelled out.\n"
        "5. REGISTER: casual, warm, spoken — like a real telecaller on the phone, NEVER stiff, formal "
        "or literary Hindi. Use \"थोड़ी देर रुकिए\" / \"एक minute hold कीजिए\", NEVER \"प्रतीक्षा "
        "कीजिए\". Use \"please\", NEVER \"कृपया\". Use \"बढ़िया\" / \"अच्छा\", not bookish words. Keep "
        "EVERY turn to one or two short sentences, then stop and listen.\n"
    )
...
class CustomerSalesAgent(Agent):
    ...
    def __init__(self, *, caller_id: str, tenant_id: str, session_id: str,
                 ...
                 pg_memory: str = "", kernel_facade=None, kernel_on: bool = False) -> None:
        # W-INT Patch C1 — customer persona via kernel (flag KERNEL_INBOUND). OFF => byte-identical.
        ...
        if kernel_on and kernel_facade is not None:
            from voice_kernel.integrations import inbound as _ki
            super().__init__(instructions=_ki.assemble_inbound_instructions(
                kernel_facade, legacy_render=_legacy, fields=fields, recap=recap,
        ...
        # W-INT — stash the kernel façade so per-turn re-renders (pick_campaign) route through it.
        self._kernel_facade = kernel_facade
        self._kernel_on = bool(kernel_on)
    ...
    async def pick_campaign(self, context: RunContext, project: str = "") -> str:
        ...
        if not RAG_INJECT_ENABLED:
            ...
            try:
                ...
                self._grounding = ""
        try:
            # W-INT Patch C2 — per-turn re-render via kernel (flag). OFF => verbatim legacy render.
            _legacy = lambda: _build_sales_instructions(
                self._fields, "", self._caller_name, False, False, None,
                grounding=self._grounding, pg_memory=self._pg_memory)
            if getattr(self, "_kernel_on", False) and getattr(self, "_kernel_facade", None) is not None:
                from voice_kernel.integrations import inbound as _ki
                await self.update_instructions(_ki.assemble_inbound_instructions(
                    self._kernel_facade, legacy_render=_legacy, fields=self._fields,
                    recap="", grounding=self._grounding, pg_memory=self._pg_memory))
            else:
                await self.update_instructions(_legacy())
        except Exception as exc:  # noqa: BLE001
...
async def _entrypoint_impl(ctx: agents.JobContext) -> None:
    ...
    # --- W-INT Patch A — inbound kernel façade slot (flag KERNEL_INBOUND, default OFF) ---------
    ...
    _camp_owner_tid = ""  # W-INT: campaign record's OWNING tenant (from contact lookup); '' => unknown
    ...
    if not is_manager:
        ...
        # W-INT: the campaign record's TRUE owning tenant (armed cross-check in Patch B).
        _camp_owner_tid = (contact.get("tenant_id") or "").strip()
    ...
    # --- W-INT Patch B — build the inbound kernel façade AFTER the tenant resolves (flag) -------
    # tenant_id here is SERVER-RESOLVED (DID/contact/ADMIN_TENANT), never a caller body field (C2).
    # campaign_tenant_id is the campaign record's OWNING tenant from the contact lookup — NOT
    # tenant_id re-passed — so session.assert_matches_campaign(...) is ARMED. On the manager /
    # no-contact path there is no separate owner, so fall back to tenant_id (consistency assert).
    if _KERNEL_INBOUND:
        try:
            from voice_kernel.integrations import inbound as _ki
            _cid = str((cust_fields or {}).get("_campaign_id", "")) if cust_fields else ""
            _camp_owner = (_camp_owner_tid or "").strip() or tenant_id
    ...
    # --- W-INT Patch D — provider/TTS seam (the Sarvam fix), flag KERNEL_INBOUND ---------------
    if _KERNEL_INBOUND and _IK is not None:
        ...
        _prov_lock_on = True                   # kernel-resolved triple is authoritative for metering
    else:
    ...
    if is_manager:
        agent = ManagerAgent(caller_id=caller_id, tenant_id=tenant_id, role=role,
                             ...
                             kernel_facade=_IK, kernel_on=_KERNEL_INBOUND)  # W-INT
        ...
        if RAG_INJECT_ENABLED and _kb is not None and cust_fields and (cust_fields.get("_campaign_id") or ""):
            async def _prefetch_grounding() -> None:
                try:
                    ...
                    # W-INT Patch C2 (twin) — route through kernel under flag; OFF => verbatim legacy.
                    _legacy = lambda: _build_sales_instructions(
                    ...
                    if _KERNEL_INBOUND and _IK is not None:
                        from voice_kernel.integrations import inbound as _ki
                        await agent.update_instructions(_ki.assemble_inbound_instructions(
                            _IK, legacy_render=_legacy, fields=agent._fields,
                            recap=agent._recap, grounding=block, pg_memory=""))
                    else:
                        await agent.update_instructions(_legacy())
    ...
    # --- W-INT Patch E — per-turn HOT hook (RAG + language), SHADOW-safe (flag KERNEL_INBOUND) --
    # ON only: compute/log the L5 rag_suffix in SHADOW (NO behavior change until a pre-LLM
    # add_message hook is wired — documented W5 deferral). OFF => never registered => hot path
    # unchanged.
    if _KERNEL_INBOUND and _IK is not None:
        @session.on("user_input_transcribed")
        def _on_turn_kernel_shadow(ev) -> None:  # noqa: ANN001
            try:
                if not getattr(ev, "is_final", True):
                    return
                _utxt = (getattr(ev, "transcript", "") or "").strip()
                if not _utxt:
                    return
                from voice_kernel.integrations import inbound as _ki
                asyncio.run_coroutine_threadsafe(
                    _ki.on_turn(_IK, user_text=_utxt, detected_lang="",
                                history_len=len(list(getattr(_slog, "_turns", [])))),
                    _loop)
            except Exception:  # noqa: BLE001 — shadow turn hook must NEVER affect the live turn
                pass
    ...
    def _on_room_disconnected_log(*_a) -> None:  # noqa: ANN002
        ...
        if _lead_memory is not None and caller_id:
            try:
                ...
                pass
        # W-INT Patch F — post-call memory write via kernel (flag). OFF => only legacy enqueue runs.
        if _KERNEL_INBOUND and _IK is not None:
            try:
                from voice_kernel.integrations import inbound as _ki
                asyncio.run_coroutine_threadsafe(
