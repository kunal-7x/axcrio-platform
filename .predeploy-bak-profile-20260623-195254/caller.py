...
# LOGGING (best-effort, defined FIRST so it is the INNERMOST wrapper around the route): record
# any UNHANDLED route exception as a system event, then RE-RAISE so Starlette's normal 500
# handling stays byte-identical. It never alters a successful response, never swallows an
# error, and is a no-op when logging_service is unconfigured — so it cannot break a request.
@app.middleware("http")
...
# SAME file so the panel surfaces backend + call failures together). Best-effort.
if _log_mod is not None:
    try:
        _log_mod.init(VAR / "system_events.jsonl")
    except Exception:  # noqa: BLE001
        pass
...
async def admin_log_suggest(request: Request, event_id: str):
    ...
    suggestion = await asyncio.to_thread(_log_mod.suggest_fix, event_id)
