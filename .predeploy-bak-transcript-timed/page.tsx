...
function CallDetailModal({
    ...
    const karaokeReady = !!recordingUrl && turns.length > 0 && timed.length === turns.length;

    // Which turn is "active" for the current playhead — the last turn that has begun.
    const activeIdx = useMemo(() => {
        if (!karaokeReady) return -1;
        for (let i = timed.length - 1; i >= 0; i--) {
            if (playhead + 0.05 >= timed[i].t0) return i;
        }
        return -1;
    }, [karaokeReady, timed, playhead]);
