...
# A1: other Indic scripts the realtime TTS (flash_v2_5) CANNOT speak. We still UNDERSTAND
# them (STT auto-detect transcribes the real script + the LLM reads it) but must DEGRADE the
# reply to Hindi audio (never silence). Counted into one `indic` bucket.
_GURMUKHI = (0x0A00, 0x0A7F)     # Punjabi (Gurmukhi)
_BENGALI = (0x0980, 0x09FF)      # Bengali/Assamese
_ORIYA = (0x0B00, 0x0B7F)        # Odia/Oriya — Sarvam STT mis-scripts Hindi 'haan' as 'ହଁ'
_TAMIL = (0x0B80, 0x0BFF)        # Tamil
_TELUGU = (0x0C00, 0x0C7F)       # Telugu
_KANNADA = (0x0C80, 0x0CFF)      # Kannada
_MALAYALAM = (0x0D00, 0x0D7F)    # Malayalam
...
def _script_counts(text: str):
    """Return (devanagari, gujarati, latin, indic) letter counts. Cheap single pass.

    `indic` = Gurmukhi/Bengali/Odia/Tamil/Telugu/Kannada/Malayalam — scripts our realtime
    TTS can't speak; counted so classify_text can DEGRADE them to a Hindi reply (understood,
    but spoken in Hindi audio). Sarvam STT frequently mis-scripts a spoken Hindi 'haan' into
    Gurmukhi 'ਹਾਂ' / Odia 'ହଁ' — both land here and route to Hindi (never echoed/acted on raw).
    """
    dev = guj = lat = indic = 0
    for ch in text:
        o = ord(ch)
        if _DEVANAGARI[0] <= o <= _DEVANAGARI[1]:
            ...
            guj += 1
        elif (_GURMUKHI[0] <= o <= _GURMUKHI[1]
              or _BENGALI[0] <= o <= _BENGALI[1]
              or _ORIYA[0] <= o <= _ORIYA[1]
              or _TAMIL[0] <= o <= _TAMIL[1]
              or _TELUGU[0] <= o <= _TELUGU[1]
              or _KANNADA[0] <= o <= _KANNADA[1]
              or _MALAYALAM[0] <= o <= _MALAYALAM[1]):
            indic += 1
        elif 0x41 <= o <= 0x5A or 0x61 <= o <= 0x7A:
            lat += 1
    return dev, guj, lat, indic
...
def classify_text(text: str):
    ...
    try:
        ...
        dev, guj, lat, indic = _script_counts(text)
        total = dev + guj + lat + indic
        if total == 0:
            return DEFAULT_LANG, 0.0

        # --- A1/R6: other-Indic script (Gurmukhi/Bengali/Odia/Tamil/Telugu/Kannada/
        # Malayalam) the TTS can't speak → DEGRADE to Hindi. The LLM understood the caller;
        # we reply in Hindi audio (the safe_tts clamp keeps the code 'hi', never 'gu'/'pa').
        # Two routes so SHORT mis-scripted affirmations also map (R6 bug-e fix): Sarvam
        # writes a spoken Hindi 'haan' as 2-char Gurmukhi 'ਹਾਂ' / Odia 'ହଁ', which the old
        # ≥4-char gate dropped → it fell through to a wrong (english) classification and the
        # AI echoed/acted on the wrong script. Now: (a) Indic is the DOMINANT script
        # (≥60% of letters) → Hindi at ANY length (a stray glyph among Latin/Devanagari is
        # never dominant, so this can't false-flip); OR (b) ≥4 Indic chars at ≥30% (mixed
        # utterance). Checked BEFORE Gujarati so a Punjabi/Tamil utterance routes to Hindi. ---
        if (indic >= 1 and indic >= 0.60 * total) or (indic >= 4 and indic >= 0.30 * total):
            return "hindi", min(1.0, indic / total + 0.2)
