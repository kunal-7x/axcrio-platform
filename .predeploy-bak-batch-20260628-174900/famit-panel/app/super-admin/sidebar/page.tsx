...
function serialize(items: WItem[]): NavConfig {
    ...
    return { order, hidden, labels, childOrder, parentOf, custom };
}
...
export default function SidebarBuilderPage() {
    ...
    const addItem = (sectionKey: string) => {
        ...
        const child: WChild = { key: newKey("link"), label, def: label, hidden: false, custom: true, href, catalogParent: sectionKey };
    ...
    const addSection = () => {
        ...
        setItems((p) => [
            ...p,
            { key: newKey("sec"), label, def: label, hidden: false, icon: "grid", isGroup: true, custom: true, children: [] },
        ]);
    };
