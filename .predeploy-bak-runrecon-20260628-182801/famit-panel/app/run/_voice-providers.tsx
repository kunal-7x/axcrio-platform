...
// Built-in mirror of the backend's llm_router/tiers.py TIERS. Used ONLY when the
// live GET /tiers catalogue is unavailable (the endpoint 401/500s, or the
// llm_router module is missing on the box) so the Quality-tier selector + quality
// badge NEVER collapse to an empty track.
...
export default function VoiceProviders({ campaignId, audienceCount, writable }: Props) {
    ...
    const tierMap = useMemo(() => {
        // Live catalogue wins; if /tiers is unavailable (empty/failed) we mirror the
        // backend presets so the selector + badge + headline ₹/min stay populated.
        const src =
            tiersData?.tiers && tiersData.tiers.length > 0
                ? tiersData.tiers
                : FALLBACK_TIERS;
        const m: Record<string, Tier> = {};
        src.forEach((t) => (m[t.key] = t));
        return m;
    }, [tiersData]);
    ...
    const order =
        tiersData?.order && tiersData.order.length > 0
            ? tiersData.order
        ...
        <div>
            ...
            <div className="mt-3 space-y-1.5">
                <p className="text-body-2 text-t-tertiary">
                    ...
                    next outbound call. Voices speak Hindi, English &amp; Hinglish
                    on the multilingual engine.
                </p>
                {usingFallback && (
