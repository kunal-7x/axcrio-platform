...
if APIRouter is not None:
    ...
    def register_number(request: Request, body: dict = Body(...)) -> dict:
        ...
        # R6-B1 — "ADD NUMBER DOES NOTHING" FIX. The SMS-OTP sender is dormant, so a number registered
        # via the panel stayed verified=False forever -> registry.lookup() rejected it (verified!=True)
        # -> the added number NEVER routed inbound. Since this add is performed by an AUTHENTICATED
        # tenant/admin through the panel (request-bound tenant_id, not a model/body field), ownership is
        # already established by the session — so we AUTO-VERIFY admin/tenant-added numbers by default
        # (verify_mode "voice_pin"/"admin"). The number is immediately usable + routes inbound. The
        # OTP path is preserved: set AIM_AUTO_VERIFY=0 to restore "register dormant -> /verify required",
        # and an explicit body {"verified": false} also forces the dormant path.
        import os as _os
        _auto_default = (_os.environ.get("AIM_AUTO_VERIFY", "1") or "1").strip().lower() \
            in ("1", "true", "yes", "on")
        _body_verified = (body or {}).get("verified", None)
        _verified = bool(_body_verified) if _body_verified is not None else _auto_default
        ...
        # OTP send stays available (dormant sender) for the explicit-verify flow; harmless when auto-verified.
        from .otp import sender as _otp
        res["otp"] = _otp.send(res["phone"])
        res["auto_verified"] = bool(_verified)
        return res
    ...
    def _aim_live_snapshot(tid: str) -> dict:
        ...
        try:
            ...
            try:
                ...
                pass
            # R6-B3 — DURABLE DB FALLBACK so the Try-it chatbot reads REAL data even when the W14
            # reporting read-model is empty (it is hydrated only from the live W8 stream; today/after a
            # restart it is frequently zero — same root cause as the dashboard B6 gap). The flat-file
            # stores (`_leads_for`, CALLS) are durable and always populated. We add a leads/calls digest
            # whenever the reporting totals look empty, so the LLM never falsely answers "0 calls".
            try:
                _tot = (snap.get("today_totals") or {})
                _hl = snap.get("hot_leads") or []
                if (not _tot or int(_tot.get("calls", 0) or 0) == 0) and not _hl:
                    _t = _c._tenant_by_id(tid) or {"tenant_id": tid,
                                                   "is_admin": (tid == getattr(_c, "ADMIN_ID", "admin"))}
                    leads = _c._leads_for(_t) or []
                    if leads:
                        bands = {"hot": 0, "warm": 0, "cold": 0}
                        for lr in leads:
                            bands[_c._lead_temp(lr)] += 1
                        snap["leads_total"] = len(leads)
                        snap["leads_by_temperature"] = bands
                        hot_rows = sorted([x for x in leads if (x.get("score", 0) or 0) >= 70],
                                          key=lambda x: x.get("score", 0) or 0, reverse=True)[:10]
                        snap["hot_leads"] = [
                            {"name": x.get("name", ""), "score": x.get("score", 0),
                             "campaign_name": x.get("campaign_name", ""),
                             "summary": x.get("ai_summary", "") or x.get("summary", ""),
                             "status": x.get("status", "")}
                            for x in hot_rows
                        ]
                    try:
                        calls = getattr(_c, "CALLS", None) or {}
                        if isinstance(calls, dict) and calls:
                            mine = [v for v in calls.values()
                                    if isinstance(v, dict) and (
                                        _t.get("is_admin") or v.get("tenant_id", "admin") == tid)]
                            if mine:
                                snap["calls_total_recent"] = len(mine)
                                snap["calls_connected_recent"] = sum(
                                    1 for v in mine if v.get("answered"))
                    except Exception:  # noqa: BLE001
                        pass
            except Exception:  # noqa: BLE001
                pass
