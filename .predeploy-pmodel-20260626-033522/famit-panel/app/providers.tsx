...
import { ClientErrorBoundary, installGlobalErrorCapture } from "@/lib/error-capture";
...
function AuthGuard({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        // `/share/*` are public customer-facing pages (e.g. a 3D property model link
        // texted mid-call) — never bounce them to /login.
        if (
            pathname === "/login" ||
            pathname === "/signup" ||
...
const Providers = ({ children }: { children: React.ReactNode }) => {
    ...
    // Capture uncaught JS errors + unhandled promise rejections app-wide -> System Logs.
    // Idempotent + best-effort; never affects rendering.
    ...
    return (
        <QueryClientProvider client={queryClient}>
            <ThemeProvider disableTransitionOnChange>
                ...
                {/* App version chip, bottom-right on every page (bump lib/version.ts). */}
                <VersionBadge />
                </ClientErrorBoundary>
            </ThemeProvider>
        </QueryClientProvider>
