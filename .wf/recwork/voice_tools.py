...
def run_campaign(campaign: str, segment: str = "all", count: int = 0) -> dict:
    ...
    form: dict = {"campaign_id": cid, "force": "1", "now": "1"}  # LPR-NOW: real dial-now (bypasses run_job window idle)
    if lead_ids:
...
def test_call(name: str, phone: str, campaign_id: str = "") -> dict:
    ...
    form = {"campaign_id": cid, "leads": f"{nm},{ph}", "force": "1", "now": "1"}  # LPR-NOW
    try:
        res = _post_form("/run", form)
    except Exception as exc:  # noqa: BLE001
...
def add_handoff(tenant_id: str, phone: str, name: str = "", priority: int = 0,
                whatsapp: str = "", hours: str = "", enabled: bool = True,
                role: str = "") -> dict:
    ...
    `name` is the person's name (used by the warm-transfer caller line: "main aapko <Name> se
    connect kar rahi hoon"); the agent passes it positionally as the 3rd arg."""
    if not tenant_id:
    ...
    cur.append({"phone": ph, "whatsapp": wa, "name": str(name or ""),
                "role": str(role or ""),
