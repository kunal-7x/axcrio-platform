"""voice_kernel.language.detector — SELF-CONTAINED per-turn language detection.

A faithful, dependency-free PORT of the LIVE OUTBOUND earner's
`droplet_work/langdetect.py` (the proven behaviour the founder asked us to port).
It is INTENTIONALLY self-contained — it imports ZERO droplet_work modules and ZERO
network/model — so the kernel-isolation guarantee + clean CI hold and the logic is
unit-testable on its own.

Goal: detect the caller's language EACH TURN from the STT transcript so the agent
can MIRROR it — steer the LLM to reply in that language AND set the TTS language.
Pure Unicode-script ratio + a small Hinglish/English marker lexicon, allocation-
cheap (HOT path), with a confidence floor + hysteresis so it never flaps.

Languages: "hindi" | "english" | "hinglish" | "gujarati" (gujarati = BETA).
Default / fallback stays "hinglish" (matches the earner).

Public API:
    classify_text(text) -> (lang, confidence)         # stateless single-shot
    LanguageTracker(default, conf_floor, min_streak)   # stateful, hysteresis-gated
        .update(text) -> (active_lang, switched: bool)
        .active                                        # current sticky language
    tts_language_code(lang) -> realtime TTS code ("hi"|"en"|"gu")
    safe_tts_language_code(lang) -> SPEAKABLE code, clamps gu->hi (never silence)
    is_speakable(lang) -> bool
    is_beta(lang) -> bool
    reply_instruction(lang) -> per-turn LLM steer string
...
# Hinglish / romanized-Hindi marker words (lowercased). If Latin text is full of
# these, it's Hinglish, not English. Kept small + high-signal to stay cheap.
...
def classify_text(text: str):
    ...
    Algorithm (identical to the live earner langdetect.classify_text):
      1. Count Devanagari / Gujarati / Latin letters.
      2. Gujarati >=2 and >=30% -> "gujarati".
      3. Both Devanagari + Latin present -> "hinglish" (code-mix).
      4. Pure Devanagari -> "hindi".
      5. Pure Latin: score words vs Hinglish vs English marker lexicons; higher
         ratio wins; tie/no markers -> low-confidence "english".
    """
    try:
        ...
        # Latin text with no strong markers (names, "ok", numbers) -> low-conf English.
        return "english", 0.25
    except Exception:
        return DEFAULT_LANG, 0.0


# Realtime TTS language codes (ISO-639-1). Hinglish speaks fine as Hindi 'hi'
# (Devanagari + Latin mix, which is what the LLM emits).
#
# HARD CONSTRAINT (verified LIVE 2026-06-05): the realtime TTS model
# `eleven_flash_v2_5` supports `hi` and `en` but NOT `gu` (Gujarati) — sending
# language_code='gu' returns unsupported_language (code 1008), the TTS websocket
# dies and the agent goes SILENT for the rest of the call (update_options is
# sticky). So we NEVER emit a code the model can't speak. Gujarati degrades to
# Hindi 'hi': we still UNDERSTAND the caller (STT auto-detect) but REPLY in Hindi
# audio (audible beats silent).
_TTS_CODE = {
    "hindi": "hi",
    "hinglish": "hi",
    "english": "en",
...
# Codes the realtime flash model can actually SPEAK. Any detect mapping outside
# this set is clamped to 'hi' (see safe_tts_language_code).
...
# Human-readable per-turn instruction the agent injects into the LLM to mirror the
# caller's language. Identical strings to the live earner (LANG_REPLY_INSTRUCTION).
LANG_REPLY_INSTRUCTION = {
    ...
    # Realtime TTS can't SPEAK Gujarati, so reply in simple Hindi (a Gujarati
    # speaker understands Hindi). Understand-Gujarati, reply-Hindi.
...
def is_speakable(lang: str) -> bool:
    """True if the realtime model can natively speak this language; else degrade."""
...
class LanguageTracker:
    ...
    Switch the active language ONLY when `min_streak` consecutive turns agree on
    the same candidate above `conf_floor`. A confident, sustained language change
    flips the agent; a stray English word inside Hinglish does not.

    INBOUND default (per the founder fix): conf_floor=0.30 (catches short English
    turns like "Hello"/"ok"), min_streak=1 (one confident turn is enough to flip)
    — exactly the V2 config the live outbound earner uses.
    """

    def __init__(self, default: str = DEFAULT_LANG, conf_floor: float = 0.30,
                 min_streak: int = 1):
        self.active = default
