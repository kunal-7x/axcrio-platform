...
_USE_CASE_PACKS: tuple[UseCasePack, ...] = (
    UseCasePack(
        ...
        closing_style="When the outcome is reached (booked / callback set / not-interested), close with ONE warm natural goodbye in the lead's own register — confirm the agreed next step in a line, thank them briefly, sign off. NEVER a canned 'ok perfect, dhanyavaad' line; never repeat the whole intro at the end.",
    ...
    UseCasePack(
        ...
        closing_style="Close by confirming the resolution or the promised follow-up time in one calm line, check they're satisfied, and sign off warmly — LLM-generated, in their language. NEVER a canned goodbye; never up-sell at the close.",
    ...
    UseCasePack(
        ...
        closing_style="Close by thanking them for their time, confirming any logged issue/follow-up, and (only if happy) leaving the door open — one warm LLM line in their register, never a scripted sign-off.",
    ...
    UseCasePack(
        ...
        closing_style="Close by reading back the confirmed slot (date+time) in one clear line, mention the reminder, and sign off warmly — LLM-generated in their language, never a canned goodbye.",
    ...
    UseCasePack(
        ...
        closing_style="Close the moment the confirmation/reschedule is captured — one short warm line acknowledging it, then sign off. Keep it brief; never a canned multi-line goodbye.",
    ...
    UseCasePack(
        ...
        closing_style="Close by thanking them sincerely for the feedback, noting any follow-up you promised, and signing off warmly — one natural LLM line, never scripted.",
    ...
    UseCasePack(
        ...
        closing_style="Close by restating the concrete next step + committed timeline, reaffirm ownership, and sign off with steadied warmth — one sincere LLM line in their register, never a canned goodbye.",
    ...
    UseCasePack(
        ...
        closing_style="Close by confirming the renewal decision (or the captured churn reason + win-back time) in one line and thanking them for their continued trust — LLM-generated, never a scripted sign-off.",
    ...
    UseCasePack(
        ...
        closing_style="Close by confirming the first successful step (or the logged blocker + follow-up time), encourage them, and sign off warmly — one natural LLM line, never scripted.",
    ...
    UseCasePack(
        ...
        closing_style="Close once the intent is served or routed — confirm the outcome/next step in one warm line and sign off, LLM-generated in their language, never a canned goodbye.",
    ...
    UseCasePack(
        ...
        closing_style="Close by reporting the action outcome (done / refused / stepped-up) crisply and asking if anything else is needed — one brief LLM line, never a scripted sign-off.",
