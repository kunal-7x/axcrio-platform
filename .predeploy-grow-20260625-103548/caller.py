...
# DEEP WIRING — register the live <60s VOICE dial into Grow. The L3 orchestrator fires it
# the instant a consent-clean lead is captured: it builds a 1-lead speed-to-lead JOB and
# schedules run_job on the FastAPI event loop via run_coroutine_threadsafe (Grow scores in a
# worker thread; grow.acapture binds the loop). WhatsApp is handled self-contained by
# grow.adapters' Graph sender — only the voice dial needs caller.py (it owns run_job/SIP/
# LiveKit). The EARNER agent.py is untouched; journey threads via the deterministic
# journey_id (lead_id = normalized phone on both capture and finalize). Reversible via flag.
if FEATURE_GROW and _grow_mod is not None:
    try:
        import asyncio as _aio_grow
        from grow.adapters import get_main_loop as _grow_main_loop
        from grow.model import ChannelResult as _GCR, ChannelStatus as _GCS

        def _grow_voice_dial(_captured, _journey):
            """Grow voice seam: 1-lead speed-to-lead job -> run_job on the main loop."""
            try:
                phone = getattr(_captured, "phone", "") or ""
                if not phone:
                    return _GCR("voice", _GCS.SKIPPED_NO_CONFIG, reason="no_phone")
                loop = _grow_main_loop()
                if loop is None:
                    return _GCR("voice", _GCS.SKIPPED_NO_CONFIG, reason="no_event_loop")
                jid = uuid.uuid4().hex[:10]
                JOBS[jid] = {
                    "state": "queued",
                    "campaign_id": getattr(_captured, "campaign_id", "") or "",
                    "tenant_id": getattr(_captured, "tenant_id", "") or "",
                    "concurrency": 1, "hourly_cap": 200, "daily_cap": 1000,
                    "force_window": False,  # honour the TRAI calling window (compliance)
                    "leads": [{"name": getattr(_captured, "name", "") or "", "num": phone,
                               "status": "queued", "room": "", "launched_at": 0.0, "attempt": 0}],
                }
                _aio_grow.run_coroutine_threadsafe(run_job(jid), loop)
                return _GCR("voice", _GCS.FIRED, ref=jid)
            except Exception as _e:  # noqa: BLE001
                return _GCR("voice", _GCS.FAILED, reason=f"dial_err:{_e!r}"[:120])

        _grow_mod.register_voice_caller(_grow_voice_dial)
    except Exception:  # noqa: BLE001 — registration must never crash the live spine
        import logging as _lg_grow2
        _lg_grow2.getLogger("famit-caller").warning("grow voice-dial registration failed",
                                                    exc_info=True)
