...
async def entrypoint(ctx: agents.JobContext) -> None:
    ...
    lang_tracker = ld.LanguageTracker(default=default_lang) if ld else None
    ...
    ctl = {"closing": False, "active_lang": default_lang, "loop": None,
           "session": None, "agent": None, "tts": None}
    logger.info("P2 lang default=%s gender=%s", default_lang, agent_gender)
    ...
    async def _apply_language_switch(new_lang: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if code not in ("hi",):  # hi already the default; en benefits from a hint
    ...
    async def _confirm_then_hangup(signal: str) -> None:
        ...
        ctl["closing"] = True
        try:
            line = _goodbye_line(signal, fields.get("agent_name") or "Riya",
                                 fields.get("company_name") or "Famit", agent_gender)
    ...
    def _on_item(ev) -> None:  # noqa: ANN001
        try:
            ...
            if text and role in ("user", "assistant"):
                ...
                loop = ctl.get("loop")
                if role == "user" and lang_tracker is not None and loop is not None and not ctl["closing"]:
                    try:
