...
BEGIN
  ...
  LOOP
    ...
    IF t IN ('ai_manager_profiles', 'ai_manager_authorized_users') THEN
      -- CONFIG / registration tables: keep the admin-GUC bypass (platform administration, not
      -- operational data). These hold engine config + the authorized-user roster.
      EXECUTE format($f$
      ...
      $f$, t);
    ELSE
      -- FULL TENANT ISOLATION on OPERATIONAL tables (sessions, session_turns, commands, audit_logs,
      -- action_runs): NO admin bypass. Even a `SET LOCAL app.is_admin='1'` session is confined to its
      -- OWN vendor_id — nobody, not even a super-admin, reads another tenant's inbound calls /
      -- transcripts / recording refs / commands. Set app.is_admin has no effect here.
      EXECUTE format($f$
        CREATE POLICY %1$s_isolation ON %1$I
