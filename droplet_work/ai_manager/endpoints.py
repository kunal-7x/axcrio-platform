...
# ---------------- full tenant isolation (mirror of caller.TENANT_HARD_ISOLATION) ----------------
def _hard_iso() -> bool:
    """Same flag + default (ON) as caller.TENANT_HARD_ISOLATION, read via env so ai_manager needs
    no import of caller at module scope. Set TENANT_HARD_ISOLATION=0 to restore legacy admin-sees-all."""
    return os.getenv("TENANT_HARD_ISOLATION", "1").strip().lower() in ("1", "true", "yes", "on")


def _data_admin(t: dict) -> bool:
    """May this VIEWER see OTHER tenants' operational data? False under hard isolation."""
    return bool(t and t.get("is_admin")) and not _hard_iso()
