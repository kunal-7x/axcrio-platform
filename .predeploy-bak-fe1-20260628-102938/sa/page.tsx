...
import {
    ...
    getVoiceCallQuality,
    type ObsRow, type VoiceFilters, type VoiceFilterOptions, type TranscriptQuality,
} from "@/lib/api";
...
function VoicePerfInner() {
    ...
    const turns = useMemo(() => {
        const m = new Map<number, { turn: number; ts: number; stt: number; eou: number; llm: number; tts: number; in_tok: number; out_tok: number }>();
        for (const ev of timeline) {
            const ti = n(ev.turn_index);
            const cur = m.get(ti) || { turn: ti, ts: n(ev.ts_ms), stt: 0, eou: 0, llm: 0, tts: 0, in_tok: 0, out_tok: 0 };
                                    ...
                                    {(() => {
                                        ...
                                        return (
                                            <>
                                                ...
                                                {/* response-latency rollup — avg / median / p95 / total */}
