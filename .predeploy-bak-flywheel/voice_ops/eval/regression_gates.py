...
def run_all_gates(goldens=None) -> GateReport:
    ...
    # ── Haptica Flywheel HARD GATES (F-*): compliance + honesty as ELIGIBILITY, never a reward.
    #    Every flywheel-proposed change (prompt / rebuttal / bandit arm) must clear these too, so a
    #    converted-but-coercive/non-compliant candidate is BLOCKED at the same deploy gate as R1..R15.
    #    Lazy + best-effort: if the flywheel package is absent the gates are simply skipped — they can
    #    never crash the core harness (which must run with or without the self-improvement engine).
    try:
        from voice_ops.flywheel import compliance as _fw_compliance  # noqa: PLC0415
        rep.results.append(_fw_compliance.gate_f_compliance(goldens))
        rep.results.append(_fw_compliance.gate_f_honesty(goldens))
    except Exception:  # noqa: BLE001 — flywheel optional; never break the core gate run
        pass
    return rep
