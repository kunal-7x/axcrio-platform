...
16. **Every deployed-file edit MUST start from a box-pulled `.LIVEBOX` golden, never the repo or any `_inbound_ref/*.py` scratch.** The box is always live truth; the repo can be weeks behind (proven: `aim_voice_agent.py` local `a9eefa8c` vs box `018c20a7` after W4+RAG). SCP direction is box→local (pull); never local→box blind. If `*.LIVEBOX.py` md5 ≠ box md5, re-pull before editing.
17. **One authoritative copy per file.** `droplet_work/<file>.LIVEBOX.py` = the canonical local mirror; `_inbound_ref/*.DEPLOYED.py` is deprecated scratch. Update `RECOVERY-STATE.md` after every deploy so the canonical source map stays current. A stale golden is worse than no golden (false confidence).
