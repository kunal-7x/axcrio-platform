...
def build_router(resolve_tenant: "Any", can: "Any", need_auth: "Any",
                 forbidden: "Any", firewall: Any = None) -> Any:
    ...
    async def _book_ep(request: "Request"):  # type: ignore[name-defined]
        ...
        p = await _body(request)
        return core.book(
            _tid(t), p.get("resource_id", ""), p.get("phone", ""),
            slot_start=p.get("slot_start"), slot_end=p.get("slot_end"),
            name=p.get("name", ""), title=p.get("title", ""), notes=p.get("notes", ""),
