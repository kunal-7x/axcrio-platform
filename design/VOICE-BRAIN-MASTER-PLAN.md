...
|---|---|---|---|
| **P0-LEAK** (security hotfix, standalone, FIRST) | tenant-prefix memory + WA paths; `_unrouted` for unknown WA; legacy read-fallback; fix hardcoded "Riya" | none (pure fix) | redeploy caller.py + memory.py to box |
...
| **W2 — Context cache + pooled httpx** | `context_store.py` LRU+Redis+version-stamp invalidation; deploy pooled `voice_tools.py` | `CTX_CACHE` | new module + voice_tools to box |
