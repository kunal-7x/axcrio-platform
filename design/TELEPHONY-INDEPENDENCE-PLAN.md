...
| **T3** (mount, flag OFF) | Additive `/trunk-registry/*` + `/trunks/byo/*` guarded mount; `/test-call` rate-limited (≤3/hr); DELETE soft-disables + refuses `_global`/env trunk; `POST /quarantine-did` kill switch | `caller.py` (additive mount only) | OFF | routes 401 unauth; flag-off → 404 dormant; legacy `/run` path intact; earner gate green | unmount / flag off |
