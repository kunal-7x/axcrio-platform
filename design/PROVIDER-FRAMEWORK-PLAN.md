...
| **W4** ✅ DONE + LIVE (flag ON) | **FW: mount endpoints in caller.py** ⚠ caller.py | 16 routes prefix `/provider-registry` CRUD + credential + reveal (step-up, single-use jti) + SSRF-guarded test + health + super-admin surfaces | `caller.py:7309-7349` (additive); `endpoints.py` (NEW); `schema.py` (UUID stringify fix for psycopg2 RowMapping) | `PROVIDER_REGISTRY_ENABLED=1` (SET ON BOX 2026-06-14) | 6-check live verify PASS: (1) routes 401 not 404; (2) create hosted-api def + AAD-encrypted cred stored (no plaintext in DB/logs); (3) SSRF blocks 169.254.169.254 + 127.0.0.1; (4) reveal requires step-up, single-use (replay→403); (5) cross-tenant RLS (axcrio cannot see/update admin's provider, each sees only own); (6) flag OFF→routes 404/legacy-providers-intact; earner gate: agent.py `9150fabe` UNCHANGED + famit-agent PID 1477083 NOT restarted + /health 200 + 0 5xx + NO ring | flag→0 (instant, routes→404) |
| **W5** ✅ DONE + DEPLOYED (2026-06-14) | **FW: strangler cut-over — VIDEO first** | `media_gen/video/client._resolve_key` asks the registry, falls back to `config.fal_key(...)` on miss | `media_gen/video/client.py` (`config.py:67-77` NEW `registry_for_video()`; `client.py:304-355` registry-then-fallback dispatcher) | `REGISTRY_FOR_VIDEO` (NOT in .env → flag-OFF resting byte-identical) | registry_for_video()=False verified on box; ON+ok→registry; ON+miss/raise/empty→legacy (6/6 offline PASS); famit-caller PID 2774834 /health 200; agent.py `9150fabe` UNCHANGED; 0 5xx; commit `a71b87a` on `fe/unify-run-wavec` | flag→0 (env delete) |
| **W6** ✅ DONE + DEPLOYED (2026-06-14) | **VID: seam fix + PG schema + live-library bridge** (U1+U2+U3) | 1-line `engine.py:46` seam fix; `ai_asset` +8 video cols + `video_jobs` + `video_scripts` (FORCE-RLS); `register_video_asset` + `?media_type=` + `/poster` on `:8310` | `creative/video_studio/engine.py`; `db/ddl_video.sql` (md5 `137c5ebc`); `ai_asset/store.py`+`endpoints.py` | `FEATURE_VIDEO_LIBRARY` (default OFF) | engine_name()→`media_gen.video.client`; 22 offline tests green; video_jobs+video_scripts in PG (RLS=t FORCE=t); ai_asset_assets has 8 new cols (media_type default 'image' → resting byte-identical); live bridge probe PASS (register ok, media_type filter, cross-tenant RLS blocked); commit `49d3165` | flag→0; tables additive (drop to remove) |
| **W7+W8** ✅ DONE + DEPLOYED (2026-06-14) | **VID: composite tier + submit_gate + caller.py mount** (U5+U4) ⚠ caller.py | `compose.py` (FFmpeg FLOOR, $0-gen-key, Sarvam $0.0023 vs EL $0.1523/clip, fixed-argv no-shell, 4-rung ABR/H.265); `submit_gate.py` (1-paid-test choke + daily cap); endpoints.py token-deriving (cross-tenant body-hole closed); caller.py mount (FEATURE_VIDEO_STUDIO, additive 0-deletion) | `media_gen/video/compose.py`; `creative/video_studio/submit_gate.py`+`endpoints.py`+`service.py`; `caller.py:7349+` | `FEATURE_VIDEO_STUDIO=0` (flag OFF resting) | gate(compose,free)=reason:free_floor (PASS); paid class fal→forced size=1/dur6/no-approve; dormant: /creative/video 404; flag-ON: routes 401 (auth-gated NOT 404) + campaigns 200 (real data); earner gate PASS; commit `ef95422` | flag→0 instant; restore VIDW8bak backups + restart famit-caller |
| **W9** ✅ DONE + GREEN (FE only, NOT deployed — 2026-06-14) | **VID: frontend** (U6) | Extend Asset type (+6 video cols); split AssetImage→AssetMedia (`<video preload="none" poster>`); FilterRail +Video; LibraryGallery +Images↔Videos toggle; `app/creative/video/page.tsx` + TierTabs (Composite default/free) + VideoCreatePanel + BatchProgress (approval gate) + ByoKeyPicker + UploadClip; lib/video.ts typed client; nav +Video Studio | `famit-panel/lib/assets.ts`+`lib/video.ts`, `app/creative/_components/AssetMedia.tsx`, `app/creative/video/*`, `FilterRail`/`LibraryGallery`/`AssetCard`/`AssetDetail`/`globals.css`/`navigation.tsx` | `FEATURE_VIDEO_STUDIO` (FE dormant-safe: 404→DormantCard) | `npm run build` exit 0 ("Compiled successfully", 58/58 static); `tsc --noEmit` 1 PRE-EXISTING error in sibling integrations page (build tolerates); gitleaks 0; zero raw hex; PANEL NOT DEPLOYED (ships in the single final deploy after voice-core-surgery); commit `2d26c98` on `fe/unify-run-wavec` | revert FE deploy (FORTRESS backup) |
