...
function VoiceDefaultsInner() {
    ...
    // #20 Stack Builder: per-tier shown-price draft (overrides est_inr_per_min via /admin/tier-config).
                ...
                <div className="grid grid-cols-3 gap-3 max-lg:grid-cols-1">
                    {tiers.map((t) => (
                        <div key={t.key} className="rounded-3xl bg-b-surface1 ring-1 ring-s-subtle ring-inset p-5 dark:bg-shade-04/30">
                            ...
                            {/* #20 Stack Builder: shown-price override + availability toggle (tier overrides) */}
                            <div className="mt-3 pt-3 border-t border-s-subtle space-y-2">
                                <div className="flex items-center gap-2">
                                    <span className="flex-1 text-caption text-t-tertiary">Shown price</span>
                                    <span className="text-caption text-t-tertiary">₹</span>
                                    <input className="input-base h-8 w-20 rounded-lg px-2 text-caption tabular-nums" inputMode="decimal"
                                        value={priceDraft[t.key] ?? String(t.est_inr_per_min)}
                                        onChange={(e) => setPriceDraft({ ...priceDraft, [t.key]: e.target.value.replace(/[^0-9.]/g, "") })} />
                                    <span className="text-caption text-t-tertiary">/min</span>
                                    <button disabled={busy}
                                        onClick={() => save({ tiers: { [t.key]: { est_inr_per_min: Math.max(0, parseFloat(priceDraft[t.key] ?? String(t.est_inr_per_min)) || 0) } } }, `${t.name} price saved`)}
                                        className="h-8 px-2.5 rounded-lg bg-primary-01 text-caption text-white disabled:opacity-50">Save</button>
                                </div>
                                <div className="flex items-center justify-between gap-2">
                                    <span className="text-caption text-t-tertiary">Available to clients</span>
                                    <button disabled={busy}
                                        onClick={() => save({ tiers: { [t.key]: { available: t.available === false } } }, `${t.name} ${t.available === false ? "enabled" : "made unavailable"}`)}
                                        className={`h-7 px-3 rounded-full text-caption font-medium ${t.available === false ? "bg-[#FF6A55]/15 text-[#FF6A55]" : "bg-[#00A656]/15 text-[#00A656]"}`}>
                                        {t.available === false ? "Unavailable" : "Available"}
                                    </button>
                                </div>
                            </div>
                        </div>
