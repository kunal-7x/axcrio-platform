...
export const CURATED_VOICES: CuratedVoice[] = [
    {
        // #21 Haptica default — the Standard-tier voice (see tiers.py). Listed FIRST + flagged so the
        // gallery opens on it and it carries the "Haptica Recommended" mark, whether it arrives via the
        // live ElevenLabs list (enriched by VOICE_META) or this fallback set. Hindi-first, female.
        voice_id: "QTKSa2Iyv0yoxvXY2V8a",
        name: "Neha",
        accent: "Indian",
        gender: "female",
        age: "Young",
        persona: "Warm Hinglish — Haptica default",
        blurb: "Natural Hindi-English warmth tuned for Indian outbound calls — our Standard-tier pick.",
        tags: ["Hinglish", "Warm"],
        recommended: true,
    },
    {
...
const GRADIENT_BY_ID: Record<string, string> = Object.fromEntries(
    // wrap on GRADIENT_COUNT so the catalogue can exceed 16 voices without 404-ing a missing image
    CURATED_VOICES.map((v, i) => [v.voice_id, `/voice-gradients/g${(i % GRADIENT_COUNT) + 1}.jpg`])
);
