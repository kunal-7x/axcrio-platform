...
import { FlowStep, type ToastFn } from "../_shared";
import BudgetPanel from "./_budget-panel";
import {
    FunnelDiagram,
    FUNNEL_LABEL,
    FUNNEL_COLORS,
...
export default function CommandTab({
    ...
    return (
        <div className="space-y-3">
            {/* Hero KPI strip — config + spend signals. Restyled to the shared
                KpiCard: restrained glyph tones (clay only where it carries meaning,
                neutral elsewhere) — no per-card colour glows. Money card carries a
                spend-vs-cap meter so the focal signal reads at a glance. */}
            {loading && !hc ? (
                ...
                <div className="grid grid-cols-4 gap-3 max-lg:grid-cols-2 max-sm:grid-cols-1">
                    <KpiCard
                        label="Ad Platforms"
                        icon="promote"
                        tone={anyProvider ? "info" : "neutral"}
                        value={
                            <span className="text-h5">
                                {anyProvider
                                    ? [metaOn && "Meta", googleOn && "Google"].filter(Boolean).join(" + ")
                                    : "Not connected"}
                            </span>
                        }
                        sub={anyProvider ? "Marketing API linked" : "Awaiting Meta / Google credentials"}
                    />
                    <KpiCard
                    ...
                    <KpiCard
                        ...
                        meter={dailyCap > 0 ? (liveSpend ? spendTodayMinor : 0) / dailyCap : null}
                        sub={
                            dailyCap > 0
                    ...
                    <KpiCard
                        ...
                        value={
                            <span className="text-h5">
                                {hc?.require_approval ? "Approval + breaker" : "Breaker"}
                            </span>
                        }
                        sub={
                            hc
            ...
            {/* Ad-budget wallet — fund / balance / ledger. Always shown: funding is
                a primary action whether the analytics surface is live or dormant. */}
            <BudgetPanel currency={currency} writable={writable} toast={toast} />
