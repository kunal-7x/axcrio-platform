...
export const saveWorkflow = (id: string, def: WfDefinition) =>
    write<{ ok: boolean; version: number }>(
        `/workflows/${encodeURIComponent(id)}`,
        { draft: def },
        "PUT"
    );

// Static validator + dominator compile. Returns offending node_ids on failure.
export type ValidateResult = { ok: boolean; code?: string; node_ids?: string[]; message?: string };
...
// Freeze the current draft as a published version (refused unless dominator-valid).
export const publishWorkflow = (id: string) =>
    write<{ ok: boolean; version: number }>(`/workflows/${encodeURIComponent(id)}/publish`, {});
