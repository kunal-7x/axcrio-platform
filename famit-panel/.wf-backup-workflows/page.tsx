...
export default function WorkflowStudioPage() {
    ...
    // The definition currently loaded in the Canvas Studio editor. Defaults to the
    // on-spec sample; Templates "Edit" loads a template's graph here + switches tab.
    const [editDef, setEditDef] = useState<WfDefinition>(SAMPLE_WORKFLOW);
        ...
        <Layout title="Workflow Studio">
            ...
            {tab === "studio" && (
                <StudioTab
                    ...
                    editDef={editDef}
                    onNewWorkflow={() => openInEditor(SAMPLE_WORKFLOW)}
                    writable={writable}
                    toast={showToast}
                />
            )}
...
function StudioTab({
    ...
    return (
        <div className="space-y-3">
            ...
            {/* The live node editor — the premium centerpiece */}
            <Card
                title={editDef.name}
                headContent={
                    <span className="ml-3 inline-flex items-center gap-2">
                        ...
                        {writable && (
                            <button
                                ...
                                className="ml-1 inline-flex items-center gap-1.5 h-8 px-3 rounded-full border border-s-subtle text-button text-t-secondary fill-t-secondary transition-colors hover:border-s-highlight hover:text-t-primary hover:fill-t-primary"
                            >
